...
class PrincipalConsultasView:
    ...
    def btn_oscuro(self, texto, expand=0, on_click=None, icono=None):
        return ft.ElevatedButton(
            ...
            expand=expand, on_click=on_click, icon=icono
        )
    ...
    def build(self):
        ...
        if getattr(session, 'is_replica', False):
            if getattr(session, 'is_read_only', False):
                ...
                color_texto = ft.Colors.ORANGE_800

        header = ft.Container(
            content=ft.Row([
                ft.Text(titulo, size=22, weight="bold", color=color_texto),
                ft.Row([
                    ft.ElevatedButton("Refrescar", icon=ft.Icons.REFRESH, on_click=lambda _: self.navegar("/ventana_principal_consultas"), bgcolor=self.COLOR_MARINO, color="white"),
        ...
        alertas = ft.ResponsiveRow([
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Column([ft.Text(f" {e['cliente']} ({e['producto']})", size=11) for e in entregas] if entregas else [ft.Text("Sin entregas", size=11)], scroll=ft.ScrollMode.AUTO, expand=True),
                ]), col=4, height=250, bgcolor="#E8F5E9", padding=12, border_radius=10
            ),
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Column([ft.Text(f" {p['producto']}: {p['stock']} pz", color="white", size=11) for p in stock_bajo] if stock_bajo else [ft.Text("Stock OK", color="white", size=11)], scroll=ft.ScrollMode.AUTO, expand=True),
                ]), col=4, height=250, bgcolor=self.COLOR_ROJO, padding=12, border_radius=10
            ),
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Column([ft.Text(f" {c['producto']}: {c['dias_restantes']} días", color="white", size=11) for c in caducidad] if caducidad else [ft.Text("Sin riesgos", color="white", size=11)], scroll=ft.ScrollMode.AUTO, expand=True),
                ]), col=4, height=250, bgcolor="#D35400", padding=12, border_radius=10
            )
        ...
        seccion_negocio = ft.ResponsiveRow([
            ft.Container(
                content=ft.Column([
                ...
                ]), col=4, height=250, bgcolor="#F4F6F7", padding=15, border_radius=10, border=ft.Border.all(1, "#DDDDDD")
            ),
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Container(content=estrellas_ui, expand=True)
                ]), col=4, height=250, bgcolor=self.COLOR_FONDO_CARTA, padding=15, border_radius=10
            ),
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Container(content=rentabilidad_ui, expand=True)
                ]), col=4, height=250, bgcolor=self.COLOR_FONDO_CARTA, padding=15, border_radius=10
            )
        ], spacing=15, run_spacing=15)
            ...
            controls=[
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Container(
                            content=ft.Row([
                                self.btn_oscuro("Historial de Ventas", expand=1, on_click=lambda _: self.navegar("/consulta_ventas"),icono=ft.Icons.ACCOUNT_BALANCE_WALLET_OUTLINED),
                                self.btn_oscuro("Historial de Cortes de Caja", expand=1, on_click=lambda _: self.navegar("/consulta_corte_caja"),icono=ft.Icons.ATTACH_MONEY_OUTLINED),
                                self.btn_oscuro("Registro de Auditoría", expand=1, on_click=lambda _: self.navegar("/consulta_auditoria"),icono=ft.Icons.PERSON_SEARCH_OUTLINED),
                                self.btn_oscuro("Catálogos (CRUD)", expand=1, on_click=lambda _: self.navegar("/crud_catalogos"),icono=ft.Icons.TABLE_VIEW)
                            ], spacing=5),
