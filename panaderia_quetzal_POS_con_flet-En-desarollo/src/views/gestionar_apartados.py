...
class GestionarApartadosView:
    ...
    def mostrar_dialogo_pago(self, apartado):
        ...
        if not caja_id:
            self.mostrar_mensaje("⚠️ Debes abrir caja para cobrar", "orange")
            return

        saldo = float(apartado['saldo_pendiente'])
        txt_monto = ft.TextField(label="Monto a Pagar ($)", value=str(saldo), width=200)
        ...
        def procesar_pago(e):
            try:
                ...
                self.cerrar_dialogo()
                self.mostrar_mensaje("✅ Apartado liquidado/abonado con éxito", "green")
                self.cargar_datos()
            except Exception as ex:
    ...
    def mostrar_dialogo_cancelar(self, apartado):
        def confirmar_cancelacion(e):
            try:
                ...
                self.cerrar_dialogo()
                self.mostrar_mensaje("Apartado cancelado exitosamente", "green")
                self.cargar_datos()
            except Exception as ex:
                self.cerrar_dialogo()
                self.mostrar_mensaje(f"Error al cancelar: {ex}", "red")
            self.page.update()
